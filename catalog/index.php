...
<?
	...
	// 6 в ряд — по аналогии с 169.ru (сравнение выдач, задача Алексея/Сергея 06.09.2026).
	// PAGE_ELEMENT_COUNT остаётся кратным этому числу (6 полных рядов), чтобы на последней
	...
	// "Actus 5.2 L" должны сравниваться как ВЕРСИОННЫЙ номер (major.minor, 10 > 2), а не как
	// десятичная дробь (5.10 == 5.1 < 5.2 — так строка ПРАВИЛЬНО ломала бы порядок ещё раз в
	// обратную сторону) и не посимвольно как строка (иначе "5.10" лексикографически встаёт
	// раньше "5.2"). Буквенный суффикс (Л/П — сторона открывания) на порядок не влияет вовсе:
	// при равном major/minor элементы равны по этому критерию, дальше решает обычный тай-брейк
	// по ID (как и у остальных полей сортировки). major*100000+minor даёт сравнимое число без
	// переписывания остального компаратора (тот работает со скалярным SORT через <=>).
	...
	function eportaExtractModelNumber(string $name): float {
		...
		return (float)($major * 100000 + $minor);
	}
	}
	...
	// Легаси-пути прежней версии сайта (до нашего вмешательства): любой путь под /catalog/,
	// который не сам корень и не распознанная коллекция (блок выше), — например
	// /catalog/vhodnie/ (раздел "Входные" сейчас существует только как ?category=entrance,
	// а не физический путь) или /catalog/mejkomnatnie/vstavki/filter/clear/ (SEF смарт-фильтра
	// старого шаблона — в eporta фильтрация только через GET-параметры, отдельного URL-пути нет).
	// Раньше такие пути молча проваливались в дефолтный рендер каталога межкомнатных с кодом
	// 200 (дубли контента под чужим адресом — жалоба робота Яндекса 09-14.09.2026); теперь —
	// настоящий 404, той же брендированной страницей, что и у отсутствующего товара.
	// Проверено (20.09.2026): нигде в шаблоне eporta нет ссылок на пути такого вида — только на
	// /catalog/, /catalog/collections/<code>/ и ?category=/?sale=1/?new=1 через сам корень.
	if ($isEportaTemplate && !$isEportaProductDetail && !$eportaCollectionSection
		&& rtrim($eportaReqPath ?? "/catalog/", "/") !== "/catalog") {
		CHTTP::SetStatus("404 Not Found");
		$eporta404Heading = "Такой страницы не существует";
		$eporta404Text = "Этот раздел был устроен иначе на прежней версии сайта — адрес изменился. Загляните в актуальный каталог.";
	...
	if ($eportaCollectionSection) {
		...
		$eportaModelRepRes = \CIBlockElement::GetList(
			...
			false, false,
			["ID", "NAME", "CODE", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "DETAIL_PICTURE", "CATALOG_PRICE_1", "PROPERTY_MODEL", "PROPERTY_RATING", "PROPERTY_SHOWCASE", "PROPERTY_COATING_COLOR"]
		);
		$eportaModelBest = [];
		...
		while ($eportaModelRow = $eportaModelRepRes->Fetch()) {
			...
			$eportaModelRowColor = (string)($eportaModelRow["PROPERTY_COATING_COLOR_VALUE"] ?? "");
			if ($eportaModelRowColor !== "" && !isset($eportaModelSwatches[$eportaModelKey][$eportaModelRowColor])) {
				$eportaModelRowPhotoId = $eportaModelRow["PREVIEW_PICTURE"] ?: $eportaModelRow["DETAIL_PICTURE"];
				$eportaModelSwatches[$eportaModelKey][$eportaModelRowColor] = [
					"url" => $eportaModelRow["DETAIL_PAGE_URL"] ?: ($eportaModelRow["CODE"] ? "/catalog/" . $eportaModelRow["CODE"] . ".html" : ""),
					"photo" => $eportaModelRowPhotoId ? \CFile::GetPath($eportaModelRowPhotoId) : "",
					"color" => $eportaModelRowColor,
				];
			}
		...
		foreach ($eportaModelBest as $eportaModelKey => $eportaModelCard) {
			...
			// Свой цвет представителя (уже показан на фото) — не дублируем кружком, тот же
			// принцип, что в catalog.section/.default/template.php. Максимум 5 кружков + "+N".
			$eportaModelAllSwatches = $eportaModelSwatches[$eportaModelKey] ?? [];
			if ($eportaModelCard["COLOR"] !== "") {
				unset($eportaModelAllSwatches[$eportaModelCard["COLOR"]]);
			}
			$eportaModelAllSwatches = array_values($eportaModelAllSwatches);
			$eportaModelCard["SWATCHES"] = array_slice($eportaModelAllSwatches, 0, 5);
			$eportaModelCard["SWATCHES_MORE"] = count($eportaModelAllSwatches) - count($eportaModelCard["SWATCHES"]);
	...
	if ($isEportaTemplate && !$isEportaProductDetail) {
		...
		} elseif (!$eportaFilterSectionId && empty($_GET["sale"]) && empty($_GET["new"])) {
			// Корень /catalog/ без выбранной категории — SEO-лендинг под "межкомнатные двери" (см.
			// ниже $eportaSeoSlug = "internal-doors" для этого же состояния, H1/title/breadcrumb уже
			// называют его "Межкомнатные двери"), но раньше фильтр по категории тут не применялся
			// вовсе — в выдачу утекали товары других категорий (Invi/"Скрытые" в "Межкомнатных",
			// жалоба 20.09.2026). Раздвижные/Входные/Арки/Фурнитура сейчас пустые (см. заглушку
...
?>
	...
	<?$APPLICATION->IncludeComponent(
		...
		[
			...
			"CACHE_GROUPS" => "N",
			"USE_STORE" => "N",
			// Товар не найден по ELEMENT_CODE из URL (снят с продажи/переименован при импорте) —
			// раньше здесь стояло "N", и компонент молча рендерил "Элемент не найден" внутри
			// страницы с кодом 200 (soft-404, заголовок оставался дефолтным "Каталог товаров" —
			// SET_TITLE не срабатывал, раз элемент не найден). Найдено 20.09.2026 по жалобе
			// робота Яндекса на 503/битые товарные URL. "Y" — как и в соседнем dresscode:catalog
			// ниже (строка ~1435) — отдаёт настоящий 404 через 404.php.
			"SET_STATUS_404" => "Y",
			"SET_TITLE" => "Y",
			"ADD_SECTIONS_CHAIN" => "N",
			"DISPLAY_COMPARE" => "N",
