...
(function () {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		// Подстраховка поверх MutationObserver: даже с {childList,attributes,characterData,subtree} на
		// практике bkh-total иногда замирал на устаревшем значении после AJAX-пересчёта количества —
		// не удалось на 100% локализовать точный тип/момент мутации, который вендор использует для
		// финальной (осевшей) правки цены. Раз в секунду принудительно пересчитываем всё из текущего
		// состояния DOM — дёшево (renderAll идемпотентен, лишней перерисовки не будет) и гарантированно
		// устраняет любое расхождение в пределах секунды, независимо от того, поймал ли его observer.
		setInterval(function () { withPausedObservers(renderAll); }, 1000);
	});
