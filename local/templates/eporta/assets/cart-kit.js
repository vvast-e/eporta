...
(function () {
	...
	// EPORTA: вендор при пересчёте (смена количества и т.п.) ПОЛНОСТЬЮ пересоздаёт DOM — и строку
	// товара (component.js: redrawBasketItemNode — старый <tr> удаляется, вставляется новый из шаблона),
	// и блок "Итого" (fillTotalBlocks: totalNodes[i].innerHTML = totalRender). Это отменяет наши переносы
	// (relocatePrice/relocateAmount) и обнуляет наши поля в "Итого" (они пустые в самом мустаче-шаблоне,
	// наполняются только этим JS) — до тех пор, пока renderAll() не пройдёт заново. Раньше тут стоял
	// debounce 400мс из предположения, что нужно ждать анимацию цены (~300мс) — это предположение НЕ
	// подтвердилось (настоящая причина другого бага, устаревшего bkh-total, была в коллизии id в
	// getDoorLineTotal, исправлено отдельно), а сама 400мс-пауза — и есть то самое заметное "голое"
	// состояние (дёрг), которое было видно между пересозданием DOM и нашим повторным рендером. Короткий
	// debounce (50мс) только схлопывает несколько мутаций одного пересоздания в один проход, не ждёт
	// никакой анимации.
	var renderTimer = null;
	function scheduleRenderAll() {
		if (renderTimer) clearTimeout(renderTimer);
		renderTimer = setTimeout(function () { renderTimer = null; withPausedObservers(renderAll); }, 50);
	}
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		// Редкая подстраховка на случай непредвиденной гонки (не основной путь реакции — им теперь
		// служит короткий debounce в scheduleRenderAll, см. комментарий там). Настоящая причина
		// устаревавшего bkh-total была найдена и исправлена отдельно (коллизия id в getDoorLineTotal).
		setInterval(function () { withPausedObservers(renderAll); }, 3000);
	});
