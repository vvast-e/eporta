...
// EPORTA: это override компонента bitrix:sale.basket.basket ("eporta" вместо ".default") —
// переопределяем только template.php + js-templates (визуал/структура), а тяжёлые/немодифицированные
// вендорские ассеты (JS-контроллер BX.Sale.BasketComponent, темы, картинки) НЕ копируем и НЕ форкаем,
// а подключаем напрямую из штатной .default-директории компонента по абсолютному пути — так они
// продолжают обновляться вместе с модулем sale и не раздувают наш шаблон копией 60+ КБ вендорского JS.
$vendorTemplateFolder = '/bitrix/components/bitrix/sale.basket.basket/templates/.default';

$this->addExternalCss('/bitrix/css/main/bootstrap.css');
$this->addExternalCss($vendorTemplateFolder.'/themes/'.$arParams['TEMPLATE_THEME'].'/style.css');

$this->addExternalJs($vendorTemplateFolder.'/js/action-pool.js');
$this->addExternalJs($vendorTemplateFolder.'/js/filter.js');
$this->addExternalJs($vendorTemplateFolder.'/js/component.js');
		...
		<div class="row">
			<div class="col-xs-12">
				<div class="basket-items-list-wrapper basket-items-list-wrapper-height-fixed basket-items-list-wrapper-light<?=$displayModeClass?>"
					...
					<!-- EPORTA: апселл "С этими дверями берут" (эталон #cart, cartUpsell) — статичный мок-набор
					     под списком позиций; см. assets/kit.js/ADDON_POOL — до появления реальных SKU/услуг
					     "+" здесь не добавляет товар в корзину по-настоящему, только визуальная заготовка. -->
					<div class="cart-upsell" id="cartUpsell">
						<div class="cart-upsell-title">С этими дверями берут</div>
						<div class="cart-upsell-grid">
							<div class="cart-upsell-card">
								<div class="cart-upsell-icon">▭</div>
								<div style="flex:1">
									<div class="cart-upsell-name">Порог алюминиевый</div>
									<div class="cart-upsell-price">640 ₽</div>
								</div>
								<div class="cart-upsell-add" title="Мок — реальные допы появятся после выгрузки 1С">+</div>
							</div>
							<div class="cart-upsell-card">
								<div class="cart-upsell-icon">▭</div>
								<div style="flex:1">
									<div class="cart-upsell-name">Уплотнитель</div>
									<div class="cart-upsell-price">380 ₽</div>
								</div>
								<div class="cart-upsell-add" title="Мок — реальные допы появятся после выгрузки 1С">+</div>
							</div>
							<div class="cart-upsell-card">
								<div class="cart-upsell-icon">▭</div>
								<div style="flex:1">
									<div class="cart-upsell-name">Набор по уходу</div>
									<div class="cart-upsell-price">520 ₽</div>
								</div>
								<div class="cart-upsell-add" title="Мок — реальные допы появятся после выгрузки 1С">+</div>
							</div>
						</div>
