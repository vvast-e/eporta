	...
	<tr class="basket-items-list-item-container{{#SHOW_RESTORE}} basket-items-list-item-container-expend{{/SHOW_RESTORE}}"
		...
		{{^SHOW_RESTORE}}
			<td class="basket-items-list-item-descriptions">
				<!-- EPORTA: шапка карточки-«комплекта» (эталон #cart) — название/бейдж/сумма позиции.
				     Бейдж и сумма наполняются/пересчитываются на клиенте в assets/cart-kit.js по мере
				     выбора допов через мини-окно «Собрать комплект» (см. .basket-item-addons ниже);
				     без выбранных допов бейдж остаётся «Только полотно». -->
				<div class="basket-kit-header" data-entity="basket-kit-header" data-id="{{ID}}">
					<div class="bkh-left">
						<span class="bkh-name">{{NAME}}</span>
						<span class="bkh-badge" data-entity="basket-kit-badge">Только полотно</span>
					</div>
					<div class="bkh-total" data-entity="basket-kit-group-total">{{{SUM_PRICE_FORMATED}}}</div>
				</div>
				...
				<!-- EPORTA: зона допов позиции + вызов общей kit-модалки (assets/kit.js, assets/cart-kit.js).
				     Строки допов рендерит cart-kit.js из клиентского состояния (localStorage, ключ
				     eporta_cart_kits) — сам компонент bitrix:sale.basket.basket о допах не знает, это
				     фронтенд-надстройка (см. комментарий в assets/kit.js над ADDON_POOL: реальных
				     SKU/услуг на проде пока нет, допы в заказ Bitrix не уходят). -->
				<div class="basket-item-addons" data-kit-item="{{ID}}" data-kit-door-fit="eco"></div>
			</td>
			<?
